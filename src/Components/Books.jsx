  ...
  return (
    ...
    {/*Declaring navbar of the website which contains title, logo, searchbar and register button*/}
      ...
      <div id="body">
        ...
        {show && data.filter(item=>item.title.toLowerCase().startsWith(search.toLowerCase())).length>1 ? data.filter(item=>item.title.toLowerCase().startsWith(search.toLowerCase())).map((item)=>{
          return(
            <div key={item.id}>
              <div key={item.id} className="Books-container">
                ...
                <div id="Flex" >
                  ...
                  <div id="button">
                    ...
                    <button id="free">
                      <a target="_blank" rel="noopener noreferrer" href={item.previewLink}>Free</a>
                    </button></div>
                </div>
              </div> 
        ...
        <div style={{height:'64vh'}}>
          <h1>Sorry! No book found, Please try searching for other books.</h1>
          <h3>We are active adding books to our collection.</h3>
        </div>
      )}
    </div>
    <footer id="footer">
